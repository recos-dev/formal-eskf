...
The [requirement checklist](verification-requirements.md) defines IDs and their intended scope. The [traceability map](../formal/agent-review/traceability-map.json) links the currently registered subset to evidence artifacts. A mapping records where to review evidence; it is not evidence that the referenced proof ran or passed.
...
The map is not a complete ESKF coverage denominator. Process noise, covariance prediction, injection/reset, correction/solve, sensor models and combined-step obligations remain tracked in [proof.md](../proof.md). Unmapped code must remain visible through the [function inventory and review queue](cpp-function-inventory-and-formal-proof-review-queue.md).
...
Run the proofs and semantic audit with:
